...
def ai_chat(request):
    if request.method == 'POST':
        try:
            ...
            # Get up to 50 available products
            products = Product.objects.filter(available=True).select_related('category', 'department')[:50]
            catalog_lines = []
            for p in products:
                cat_name = p.category.name if p.category else 'Без категории'
                dept_name = p.department.name if p.department else 'Без отдела'
                catalog_lines.append(f"- ID:{p.id} | Название: {p.name} | Отдел: {dept_name} | Категория: {cat_name}")
            catalog_info = "\n".join(catalog_lines)
            
            prompt = f"""Ты — умный и вежливый виртуальный стилист магазина одежды QarStyle. 
Отвечай кратко, естественно, дружелюбно и ТОЛЬКО на русском языке.

Твоя база знаний (доступные товары в наличии):
{catalog_info}

СТРОГИЕ ПРАВИЛА (ВАЖНО!):
1. ПРИВЕТСТВИЕ: Если пользователь просто здоровается (например, "привет", "здравствуйте", "как дела") - просто поздоровайся в ответ и спроси, чем можешь помочь. НЕ предлагай товары сразу!
2. ОТСУТСТВИЕ ТОВАРА: Если пользователь просит то, чего нет в базе знаний (например, футболки, обувь, очки), честно скажи: "К сожалению, сейчас у нас нет этого в наличии. Может быть, вас заинтересуют наши актуальные коллекции?" НЕ предлагай куртки вместо футболок! Строго придерживайся названий и категорий из базы знаний.
3. ПОДБОР ТОВАРА: Если пользователь ищет то, что есть в базе знаний (например, куртка, свитер), подбери 1-2 подходящих варианта и ОБЯЗАТЕЛЬНО вставь тег [PRODUCT_ID:X], где X — номер ID товара. Пример: "У нас есть отличный вариант: [PRODUCT_ID:4]". Цены не пиши.
4. ОФФТОП: Если вопрос не по теме одежды, вежливо верни разговор к ассортименту магазина.
...
Вопрос клиента: {user_message}"""
            ...
            if not text:
                text = "Извините, сейчас я немного перегружен запросами. Пожалуйста, воспользуйтесь обычным каталогом на нашем сайте!"

            if text:
                import re
                # Matches [PRODUCT_ID:5] or (ID: 5) or ID: 5 or ID:5
